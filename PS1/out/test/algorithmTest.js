...
describe("toBucketSets()", () => {
    ...
    it("should handle single bucket with cards", () => {
        const card1 = new flashcards_1.Flashcard("Q1", "A1", "H1", []);
        const card2 = new flashcards_1.Flashcard("Q2", "A2", "H2", []);
        const bucket = new Set([card1, card2]);
        const buckets = new Map([[0, bucket]]);
        const result = (0, algorithm_1.toBucketSets)(buckets);
        assert_1.default.deepStrictEqual(result, [bucket]);
    });
    it("should handle multiple buckets with gaps", () => {
        ...
        const result = (0, algorithm_1.toBucketSets)(buckets);
        assert_1.default.deepStrictEqual(result, [bucket0, new Set(), bucket2]);
    });
    it("should handle buckets out of order", () => {
        ...
        const result = (0, algorithm_1.toBucketSets)(buckets);
        assert_1.default.deepStrictEqual(result, [bucket0, bucket1]);
    });
});
/*
...
describe("getBucketRange()", () => {
    ...
    it("should handle single non-empty bucket", () => {
        const card = new flashcards_1.Flashcard("Q", "A", "H", []);
        const buckets = [new Set(), new Set([card]), new Set()];
        const result = (0, algorithm_1.getBucketRange)(buckets);
        assert_1.default.deepStrictEqual(result, { minBucket: 1, maxBucket: 1 });
    });
    it("should handle multiple non-empty buckets", () => {
        ...
        const result = (0, algorithm_1.getBucketRange)(buckets);
        assert_1.default.deepStrictEqual(result, { minBucket: 1, maxBucket: 3 });
    });
    it("should handle non-consecutive non-empty buckets", () => {
        ...
        const result = (0, algorithm_1.getBucketRange)(buckets);
        assert_1.default.deepStrictEqual(result, { minBucket: 0, maxBucket: 3 });
    });
});
/*
...
describe("practice()", () => {
    it("should return empty set for empty buckets", () => {
        ...
        assert_1.default.deepStrictEqual(result, new Set());
    });
    it("should return cards from bucket 0 on day 0", () => {
        const card1 = new flashcards_1.Flashcard("Q1", "A1", "H1", []);
        const card2 = new flashcards_1.Flashcard("Q2", "A2", "H2", []);
        const buckets = [new Set([card1, card2]), new Set()];
        const result = (0, algorithm_1.practice)(buckets, 0);
        assert_1.default.deepStrictEqual(result, new Set([card1, card2]));
    });
    it("should return cards from bucket 1 on day 1", () => {
        const card1 = new flashcards_1.Flashcard("Q1", "A1", "H1", []);
        const card2 = new flashcards_1.Flashcard("Q2", "A2", "H2", []);
        const buckets = [new Set(), new Set([card1, card2])];
        const result = (0, algorithm_1.practice)(buckets, 1);
        assert_1.default.deepStrictEqual(result, new Set([card1, card2]));
    });
    it("should return cards from multiple practice buckets", () => {
        const card1 = new flashcards_1.Flashcard("Q1", "A1", "H1", []);
        const card2 = new flashcards_1.Flashcard("Q2", "A2", "H2", []);
        const card3 = new flashcards_1.Flashcard("Q3", "A3", "H3", []);
        const buckets = [
            new Set(),
            new Set([card1]),
            new Set(),
        ...
        assert_1.default.deepStrictEqual(result, new Set([card2, card3]));
    });
    it("should handle day number larger than bucket count", () => {
        const card1 = new flashcards_1.Flashcard("Q1", "A1", "H1", []);
        const card2 = new flashcards_1.Flashcard("Q2", "A2", "H2", []);
        const buckets = [new Set([card1]), new Set([card2])];
        const result = (0, algorithm_1.practice)(buckets, 5);
        assert_1.default.deepStrictEqual(result, new Set());
    });
});
/*
...
describe("update()", () => {
    it("should move card to bucket 0 on wrong answer", () => {
        ...
        assert_1.default.deepStrictEqual(result.get(0), new Set([card]));
        assert_1.default.deepStrictEqual(result.get(1), new Set());
    });
    it("should move card up one bucket on easy answer", () => {
        ...
        const result = (0, algorithm_1.update)(buckets, card, flashcards_1.AnswerDifficulty.Easy);
        assert_1.default.deepStrictEqual(result.get(1), new Set());
        assert_1.default.deepStrictEqual(result.get(2), new Set([card]));
    });
    it("should keep card in same bucket on hard answer", () => {
        ...
        assert_1.default.deepStrictEqual(result.get(1), new Set([card]));
    });
    ...
    it("should handle multiple cards in buckets", () => {
        ...
        const result = (0, algorithm_1.update)(buckets, card1, flashcards_1.AnswerDifficulty.Easy);
        assert_1.default.deepStrictEqual(result.get(1), new Set([card2]));
        assert_1.default.deepStrictEqual(result.get(2), new Set([card1]));
    });
});
/*
...
describe("getHint()", () => {
    it("should return empty string for empty hint", () => {
        const card = new flashcards_1.Flashcard("Q", "A", "", []);
    ...
    it("should return first character of answer for short hint", () => {
        const card = new flashcards_1.Flashcard("Q", "Answer", "H", []);
        const result = (0, algorithm_1.getHint)(card);
        assert_1.default.strictEqual(result, "A");
    });
    ...
    it("should handle hint with spaces", () => {
        const card = new flashcards_1.Flashcard("Q", "A B C", "H", []);
        const result = (0, algorithm_1.getHint)(card);
        assert_1.default.strictEqual(result, "A");
    });
    it("should handle long answer", () => {
        const card = new flashcards_1.Flashcard("Q", "This is a very long answer", "H", []);
        const result = (0, algorithm_1.getHint)(card);
        assert_1.default.strictEqual(result, "T");
    });
});
/*
...
describe("computeProgress()", () => {
    ...
    it("should calculate progress with some cards", () => {
        ...
        const history = [
            { card: card1, difficulty: flashcards_1.AnswerDifficulty.Wrong, timestamp: Date.now() },
            { card: card2, difficulty: flashcards_1.AnswerDifficulty.Hard, timestamp: Date.now() },
            { card: card3, difficulty: flashcards_1.AnswerDifficulty.Easy, timestamp: Date.now() },
        ];
        const result = (0, algorithm_1.computeProgress)(buckets, history);
        assert_1.default.deepStrictEqual(result, {
            totalCards: 3,
    ...
    it("should handle mixed difficulty history", () => {
        ...
        const history = [
            { card, difficulty: flashcards_1.AnswerDifficulty.Wrong, timestamp: Date.now() - 1000 },
            { card, difficulty: flashcards_1.AnswerDifficulty.Hard, timestamp: Date.now() - 500 },
            { card, difficulty: flashcards_1.AnswerDifficulty.Easy, timestamp: Date.now() },
        ];
        const result = (0, algorithm_1.computeProgress)(buckets, history);
        assert_1.default.deepStrictEqual(result, {
            totalCards: 1,
